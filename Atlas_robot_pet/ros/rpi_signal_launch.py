        ...
        while not rospy.is_shutdown():
            ...
            if rpi_signal_flag:
                ...
                if self.face_centered and self.received_sig  == "take_a_picture":
                        ...
                        rospy.loginfo("-----------PEEEEEEEEEEEEEEEEEEEEEKKKKKKKKKKKKKKKKKKKKKKKKKOOOOOOOOOOOOOOOOOO------------------------")
                ...
                elif self.hg_sig  == "ACTIVATE" and self.received_sig != "take_a_picture" and not object_tracking_flag:
                    rospy.loginfo("Sending: Activate")
                    self.data = pickle.dumps("activate\n", 0)

                elif self.hg_sig  == "DEACTIVATE" and self.received_sig != "take_a_picture" and not object_tracking_flag:
                    rospy.loginfo("Sending: Deactivate")
                    self.data = pickle.dumps("deactivate\n", 0)

                elif self.hg_sig  == "TAKE A PICTURE" and self.received_sig != "take_a_picture" and not object_tracking_flag:
                    rospy.loginfo("Sending: Take a Picture")
                    self.data = pickle.dumps("take a picture\n", 0)

                elif self.hg_sig  == "SPIN" and self.received_sig != "take_a_picture" and not object_tracking_flag:
                    rospy.loginfo("Sending: Spin")
                    self.data = pickle.dumps("spin\n", 0)
                ...
                if self.data == "received spin\n":
                    self.received_sig  = "spin"
                    rospy.loginfo(self.received_sig )

                if self.data == "received forward\n":
                    self.received_sig  = "forward"
                    rospy.loginfo(self.received_sig )

                if self.data == "received backward\n":
                    self.received_sig  = "backward"
                    rospy.loginfo(self.received_sig )

                if self.data == "received left\n":
