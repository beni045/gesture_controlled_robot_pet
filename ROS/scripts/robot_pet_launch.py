...
BODYPOSE_CONF = "../gesture_controlled_robot_pet/Atlas_robot_pet/param.conf"
BG_COUNTER_MAX = 3
...
class robot_pet_launch:
    def __init__(self):
        ...
        self.rotate_pub = rospy.Publisher("rotate", String, queue_size=10)
        ...
        self.send_take_a_pic_flag = False
        self.head_centering_flag = 0

        # Init parameters in case we're not using the launch file
    ...
    def pet_gestures(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.head_centering_flag = rospy.get_param("/head_centering_flag")
            rospy.loginfo(self.head_centering_flag)
            ...
            canvas, bg_command, coords, rotate = bodypose_model_processor.predict(
                img_original, img_original, self.activate_flag
            )

            # We only register a command if the same commands have been detected BG_COUNTER_MAX times in a row
            # Otherwise the command will be ignored (treated as STOP)
            if bg_command == self.curr_bg_command:
                self.bg_counter += 1
                if self.bg_counter < BG_COUNTER_MAX:
                    bg_command = "STOP"
                else:
                    self.bg_counter = 0
            else:
                self.curr_bg_command = bg_command
                bg_command = "STOP"
                self.bg_counter = 0

            # Send ACTIVATE/DEACTIVATE to the Pi
            if bg_command == "ACTIVATE" or bg_command == "DEACTIVATE":
            ...
            if self.activate_flag:
                if self.take_a_pic_flag:
                    ...
                    bg_command, rotate = self.check_nothing_count(bg_command, rotate)
                    ...
                    if width * height < 0.048 * 1280 * 720:
                        ...
                        rospy.set_param("/take_a_pic_flag", 0)
                        bg_command = "STOP"
                        rospy.set_param("/head_centering_flag", 1)

                elif self.send_take_a_pic_flag:
                    ...
                    bg_command = "TAKE A PICTURE"
                    rotate = "NOTHING"
                    self.send_take_a_pic_flag = False
                    rospy.loginfo(rotate)
                    self.rotate_pub.publish(rotate)

                elif self.head_centering_flag:
                    ...
                    if rotate == "CENTERED":
                        ...
                        if self.face_center_count < 5:
                            ...
                            self.face_center_count = 0
                            self.send_take_a_pic_flag = True

                    rospy.loginfo(rotate)
                    rospy.set_param("/rpi_signal_flag", 1)
                    self.rotate_pub.publish(rotate)

                elif bg_command == "TAKE A PICTURE":
                    if not rospy.get_param("/object_tracking_flag"):
                        # If it's not in Follow mode, inform Pi that Smart Photography routine starts
                        bg_command = "center for picture"
                        rospy.set_param("/rpi_signal_flag", 1)
                elif bg_command == "SPIN LEFT":
                    rospy.set_param("/rpi_signal_flag", 1)
                elif bg_command == "SPIN RIGHT":
                    rospy.set_param("/rpi_signal_flag", 1)
                elif bg_command == "FORWARDS":
                    rospy.set_param("/rpi_signal_flag", 1)
                elif bg_command == "STOP FOLLOW":
                    # Stop object tracking
                    rospy.set_param("/rpi_signal_flag", 1)
                elif bg_command == "FOLLOW":
                    ...
                    rospy.set_param("/rpi_signal_flag", 1)
                elif bg_command == "BACKWARDS":
                    rospy.set_param("/rpi_signal_flag", 1)
                elif bg_command == "ACTIVATE" or bg_command == "DEACTIVATE":
                    rospy.set_param("/rpi_signal_flag", 1)
                elif bg_command == "BODY":
                    # This is used to indicated body is in the frame but no valid command detected
                    rospy.set_param("/rpi_signal_flag", 1)

            else:
                # If the robot goes to deactive, reset flags/counters
                self.nothing_count = 0
                self.take_a_pic_flag = False
                self.send_take_a_pic_flag = False
                rospy.set_param("/head_centering_flag", 0)

            rospy.loginfo(bg_command)
            self.pub.publish(bg_command)
    ...
    def check_nothing_count(self, bg_command, rotate):
        if rotate == "NOTHING":
